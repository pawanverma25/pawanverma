...
function ProjectCard({ title, description, tech, github, external, image, isReverse }) {
	return (
		<div className={`relative grid items-center gap-4 ${isReverse ? 'md:grid-cols-[1fr_1.5fr]' : 'md:grid-cols-[1.5fr_1fr]'}`}>
			<div className={`relative z-10 ${isReverse ? 'md:order-2' : ''}`}>
				...
				<ul className="flex flex-wrap gap-3 mb-4">
					{tech.map((item, index) => (
						<li key={index} className="font-mono text-sm text-slate">
							{item}
						</li>
					))}
				</ul>
